...
-- Permissive policies for v1 (allow all authenticated users to read)
...
CREATE POLICY "Users can view payment net terms references" 
  ON payment_net_terms_references FOR SELECT 
  USING (net_terms_id IN (
    SELECT id FROM payment_net_terms WHERE payment_info_id IN (
      SELECT id FROM payment_info WHERE organization_id IN (
        SELECT organization_id FROM users WHERE id = auth.uid()
      )
    )
