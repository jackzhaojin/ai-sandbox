/**
 * Recipe Discovery Platform - Verified Demo
 *
 * Streamlined demo with mandatory visual verification checks
 *
 * Run: npx playwright test --config=playwright.video.config.ts --grep @verified-demo
 *
 * @tags @verified-demo
 */
...
async function verifyNoErrors(page: Page): Promise<void> {
  // MANDATORY: Check for Next.js runtime errors in body text
  ...
  await expect(page.locator('body')).not.toContainText('Invalid src prop');
}
...
test('@verified-demo Recipe Discovery Platform Demo', async ({ page }) => {
  ...
  // Verify recipe cards are visible before narrating about them
  await page.waitForSelector('article, [role="article"], .recipe-card, a[href*="/recipes/"]', { timeout: 10000 });
